...
type TicketRepositoryPostgres interface {
	...
	CheckTicketExist(trackId string, airplaneId string, date time.Time, time time.Time) error
	Delete(id string) error
	VerifyTicketAvailable(idTicket string) error
	Update(idTicket string, ticket *Ticket)(*Ticket, error)
}
...
type TicketUseCase interface {
	...
	Delete(id string) error
	Update(idTicket string, ticket *TicketReq)(*TicketRes, error)
}